...
import { getAdministrativo, updateAdministrativo } from 'redux/administrativo/thunks';
...
function Perfil() {
  ...
  const userAdministrativo = administrativos.find((user) => user.email === userCurrent);
  ...
  const {
    ...
    defaultValues: { ...userAdministrativo }
  });

  console.log(errors);

  const resetForm = () => {
    if (userAdministrativo?.tipo) {
      const admData = {
        tipo: userAdministrativo.tipo,
        nombre: userAdministrativo.nombre,
        apellido: userAdministrativo.apellido,
        dni: userAdministrativo.dni,
        fechaNacimiento: userAdministrativo.fechaNacimiento,
        fechaContratacion: userAdministrativo.fechaContratacion,
        direccion: userAdministrativo.direccion,
        localidad: userAdministrativo.localidad,
        telefono: userAdministrativo.telefono,
        contrato: userAdministrativo.contrato,
        hsLaborales: userAdministrativo.hsLaborales,
        salario: userAdministrativo.salario,
        email: userAdministrativo.email,
        fechaActualizacionSalario: userAdministrativo.fechaActualizacionSalario,
        numeroSeguridadSocial: userAdministrativo.numeroSeguridadSocial,
        oficina: userAdministrativo.oficina,
        any: userAdministrativo.any,
        departamento: userAdministrativo.departamento,
        puesto: userAdministrativo.puesto,
        cantidadHijos: userAdministrativo.cantidadHijos,
        estadoCivil: userAdministrativo.estadoCivil,
        activo: userAdministrativo.activo,
        cuentaBancaria: userAdministrativo.cuentaBancaria
      };
      reset({ ...admData });
    }
  };
  ...
  const onConfirmFunction = async () => {
    const editControladorResponse = await updateAdministrativo(
      dispatch,
      userAdministrativo._id,
      administrativo
    );
    if (editControladorResponse.type === 'UPDATE_ADMINISTRATIVO_SUCCESS') {
  ...
  const onSubmit = async (data) => {
    ...
    setAdministrativo(formattedData);
