...
import Joi from 'joi';
import { signUpRelevador } from 'redux/auth/thunks';
...
const RelevadorForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (Object.values(errors).length === 0) {
      try {
        const responseSignUp = await dispatch(signUpRelevador(relevadorEdit));
        ...
        <div className={styles.wholeContainer}>
          {openModalSuccess && (
            <ModalSuccess
              ...
              message={'El relevador ha sido creado.'}
          ...
          <div className={styles.imgTop}>
            <p className={styles.imgText}>RELEVADOR</p>
          </div>
